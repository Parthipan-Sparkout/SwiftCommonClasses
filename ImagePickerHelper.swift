...
extension ImagePicker {
    ...
    private func showAlert(targetName: String, completion: ((Bool) -> Void)?) {
        DispatchQueue.main.async { [weak self] in
            ...
            let alertVC = UIAlertController(title: "Access to the \(targetName)".localized(),
                                            message: "Please provide access to your \(targetName)".localized(),
...
extension ImagePicker: UIImagePickerControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController,
        ...
        let filenameUnwrapped = filename ?? "no_name_\(Int.random(in: 1 ..< 100))"
        self.imageName = filenameUnwrapped
